namespace ExtrudedTextHelixSharpDX
{
    ...
    {
        public static void ExtrudeText(this MeshBuilder builder, string text, string font, FontStyle fontStyle, FontWeight fontWeight, double fontSize, Vector3D textDirection, 
        ...
        public static Point3D Project(this Vertex v, Point3D p0, Vector3D x, Vector3D y, Vector3D z, float h)
        {
            return p0 + x * (float)v.X - y * (float)v.Y + z * h;
        }
        ...
        public static IEnumerable<Vector2> ToSegments(this IEnumerable<Point> input)
        {
            bool first = true;
            ...
            foreach (var point in input)
            {
                if (!first)
                {
                    yield return previous.ToVector2();
